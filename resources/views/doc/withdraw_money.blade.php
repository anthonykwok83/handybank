...
@section('footer_js')
    ...
    <script>
        ...
        function tryRun() {
            $.ajax({
                method: "POST",
                url: "{{ url('/') }}/api/v1/user/" + data.user_id + "/account/" + data.account_id + "/withdraw",
        ...
        var vm = new Vue({
            ...
            computed: {
                bashSourceCode: function () {
                    return "curl -H \"X-Requested-With: XMLHttpRequest\" -d \"amount=" + this.amount + "\" -X POST {{ url('/') }}/api/v1/user/" + this.user_id + "/account/" + this.account_id + "/withdraw";
                },
                jsSourceCode: function () {
                    return "$.ajax({\n" +
                        "    method: \"POST\",\n" +
                        "    url: \"{{ url('/') }}/api/v1/user/" + this.user_id + "/account/" + this.account_id + "/withdraw\",\n" +
